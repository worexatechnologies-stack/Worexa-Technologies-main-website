...
file_put_contents(__DIR__ . '/debug.log', date('[Y-m-d H:i:s] ') . "Script started. Request Method: " . $_SERVER['REQUEST_METHOD'] . " | POST keys: " . implode(',', array_keys($_POST)) . " | FILES keys: " . implode(',', array_keys($_FILES)) . "\n", FILE_APPEND);
...
function smtp_config_from_env($env) {
    ...
    $fromEmail = $env['MAIL_FROM'] ?? $username;
    ...
    return [
        ...
        'timeout' => (int) ($env['SMTP_TIMEOUT'] ?? 8),
    ];
}
